...
def dict_to(d, device):
    ...
    for k, v in d.items():
        ...
        elif isinstance(v, dict) or isinstance(v, transformers.tokenization_utils_base.BatchEncoding):
...
class EditBatchSampler:
    ...
    def stereosample(self, batch_size):
        if self.edit_position >= self.n:
            self._init()
        
        edit_idxs = self.perm[self.edit_position: self.edit_position + batch_size]
        self.edit_position += batch_size

        return edit_idxs.tolist()
