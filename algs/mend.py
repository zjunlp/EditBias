...
class GradientTransform(nn.Module):
    ...
    def forward(self, u, v, param_idx=None):
        ...
        if self.cfg.combine:
            ...
            out1, out2 = output.split([u.shape[-1], v.shape[-1]], -1)   # pseudoactivations and pseudodeltas
            return out1, out2
        else:
...
class MEND(EditableModel):
    ...
    def __init__(self, model, config, model_constructor, tokenizer, mend=None, edit_lrs=None):
        super().__init__(model, config, model_constructor, tokenizer)
        ...
        if not hasattr(self.model, "handles"):
            hook_model(self.model, self.config.model.inner_params)  # model.handles
    ...
    def edit(self, batch, condition=None, detach_history=False):
        # assert batch['anti']['input_ids'].shape[1]==64
        # assert batch['stereo']['input_ids'].shape[1]==64
        outputs_anti, outputs_stereo = _logits(self.model(**batch['anti'])), _logits(self.model(**batch['stereo']))
        loss = self.edit_loss_fn(outputs_anti, batch['anti']["labels"], outputs_stereo, batch['stereo']["labels"])["loss"]
        ...
        if self.config.mend.shared:
            ...
            for n, p in _inner_params(self.model.named_parameters(), self.config.model.inner_params):
                transformed_factors[n] = self.mend[str(tuple(self.get_shape(p)))](p.__x__, p.__delta__, param_idx(n, p))
        else:
            transformed_factors = {
        ...
        return MEND(edited_model, self.config, self.model_constructor, tokenizer=self.tokenizer, mend=self.mend, edit_lrs=self.edit_lrs), info_dict
