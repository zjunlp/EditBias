...
def get_tokenizer(config):
    ...
    tok_name = config.model.tokenizer_name
    return getattr(transformers, config.model.tokenizer_class).from_pretrained(tok_name, cache_dir=scr())


if __name__ == '__main__':
    m = BertClassifier("/newdisk3/data/xxu/mend/edited_BERT")
    m.classifier.load_state_dict(torch.load("/newdisk3/data/xxu/mend/edited_BERT/classifier.bin"))
    from transformers import BertTokenizer
    tokenizer = BertTokenizer.from_pretrained("/newdisk3/data/xxu/mend/bert-base-uncased")
    inputs = tokenizer("Fox 2000 Pictures released the film Soul Food.", return_tensors="pt")
    print(m())
