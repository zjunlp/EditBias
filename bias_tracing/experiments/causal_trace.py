...
def calculate_hidden_flow(
    ...
    for word in knowledge['subject']:
        if word not in knowledge['anti'] or word not in knowledge['stereo']:
            ...
            return None
    if mt.iscausal:
        inp_anti, e_range_anti, blank_idxs_anti, _ = make_inputs(mt, prompts=[knowledge['anti']] * (samples + 1), labels=[knowledge['anti_mask']] * (samples + 1), subject=knowledge['subject'])
        inp_stereo, e_range_stereo, blank_idxs_stereo, _ = make_inputs(mt, prompts=[knowledge['stereo']] * (samples + 1), labels=[knowledge['stereo_mask']] * (samples + 1), subject=knowledge['subject'])
        if (inp_anti==None and e_range_anti==None and blank_idxs_anti==None and _==None) or (inp_stereo==None and e_range_stereo==None and blank_idxs_stereo==None and _==None):
            return None
        with torch.no_grad():
        ...
        inp_anti, e_range_anti, blank_idxs_anti, inp_anti_origin = make_inputs(mt, prompts=[knowledge['anti_mask']] * (samples + 1), labels=[knowledge['anti']] * (samples + 1), subject=knowledge['subject'])
        inp_stereo, e_range_stereo, blank_idxs_stereo, inp_stereo_origin = make_inputs(mt, prompts=[knowledge['stereo_mask']] * (samples + 1), labels=[knowledge['stereo']] * (samples + 1), subject=knowledge['subject'])
    ...
    # difference after corrupting embedding: low_score
    ...
    # difference after corrupting embedding and restoring: differences
    ...
    return dict(
        scores=differences,                                             # after causal tracing to interrupt every token and restoring them, probs of the predicted object/token 
        low_score=low_score,                                            # after corrupting embedding, return the prob of the predicted object/token
        high_score=base_score,                                          # before causal tracing, the original prob of the predicted object/token
        ...
        blank_idxs_stereo=blank_idxs_stereo,                                                                                                                                      # subject idx range in input_ids
        subject=knowledge['subject'],                                                  # the mention of the predicted object/token
        window=window,
...
def plot_trace_heatmap(result, savepdf_pre=None, title=None, xlabel=None, modelname=None):
    ...
    with plt.rc_context(rc={"font.family": "Times New Roman"}):
        ...
        if xlabel is not None:
            ...
            pass
            # cb.ax.set_title(f"Absolute log probability difference", y=-0.16, fontsize=5)
        if savepdf:
